use crate::ast::Type;
use crate::tir::{
    ArithBinOp, FloatArithOp, IntArithOp, OrderedCmpOp, RawBinOp, TypedBinOp, TypedCompare,
    ValueType,
};
...
fn sequence_binop_rule(op: RawBinOp, left: &Type, right: &Type) -> Option<BinOpRule> {
    ...
    match op {
        RawBinOp::Arith(ArithBinOp::Add) => match (left, right) {
            ...
            (List(a), List(b)) if a == b => Some(BinOpRule::same(List(a.clone()))),
            _ => None,
        },
        RawBinOp::Arith(ArithBinOp::Mul) => match (left, right) {
            ...
            (List(inner), Int) | (Int, List(inner)) => Some(BinOpRule::same(List(inner.clone()))),
            _ => None,
        },
        _ => None,
    }
