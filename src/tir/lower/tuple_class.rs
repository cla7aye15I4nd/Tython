...
impl Lowering {
    ...
    fn gen_tuple_repr(&mut self, class_name: &str, elem_types: &[ValueType]) -> TirFunction {
        ...
        // Save pre_stmts — we'll use lower_repr_str_expr which may push to pre_stmts
        ...
        for (i, elem_ty) in elem_types.iter().enumerate() {
            ...
            // Generate repr expression for this element
            let repr_expr = self.lower_repr_str_expr(field_expr);
            // Collect any pre_stmts generated by nested repr
            body.append(&mut self.pre_stmts);
