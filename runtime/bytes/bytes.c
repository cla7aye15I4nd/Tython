...
TythonBytes* TYTHON_FN(bytes_new)(const uint8_t* data, int64_t len) {
    TythonBytes* b = (TythonBytes*)TYTHON_FN(malloc)(sizeof(TythonBytes));
    b->len = len;
    b->data = (uint8_t*)TYTHON_FN(malloc)(len > 0 ? len : 1);
    if (len > 0) {
...
TythonBytes* TYTHON_FN(bytes_concat)(TythonBytes* a, TythonBytes* b) {
    ...
    TythonBytes* r = (TythonBytes*)TYTHON_FN(malloc)(sizeof(TythonBytes));
    r->len = new_len;
    r->data = (uint8_t*)TYTHON_FN(malloc)(new_len > 0 ? new_len : 1);
...
TythonBytes* TYTHON_FN(bytes_repeat)(TythonBytes* s, int64_t n) {
    if (n <= 0) {
        return TYTHON_FN(bytes_new)(NULL, 0);
    }
...
TythonBytes* TYTHON_FN(bytes_from_int)(int64_t n) {
    ...
    TythonBytes* b = (TythonBytes*)TYTHON_FN(malloc)(sizeof(TythonBytes));
    b->len = n;
    b->data = (uint8_t*)TYTHON_FN(malloc)(n > 0 ? n : 1);
    memset(b->data, 0, (size_t)n);
    return b;
}

TythonBytes* TYTHON_FN(bytes_from_str)(TythonStr* s) {
    return TYTHON_FN(bytes_new)((const uint8_t*)s->data, s->len);
}
