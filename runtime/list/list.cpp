...
void TYTHON_FN(print_list_bool)(TythonList* lst) {
    ...
    for (int64_t i = 0; i < p->len; i++) {
        if (i > 0) std::printf(", ");
        std::printf("%s", p->data[i] ? "True" : "False");
    }
    std::printf("]");
}
...
void TYTHON_FN(print_list_str)(TythonList* lst) {
    ...
    for (int64_t i = 0; i < p->len; i++) {
        if (i > 0) std::printf(", ");
        auto* s = reinterpret_cast<TythonStr*>(static_cast<uintptr_t>(p->data[i]));
        std::printf("'%.*s'", static_cast<int>(s->len), s->data);
    }
    std::printf("]");
}
...
void TYTHON_FN(print_list_bytes)(TythonList* lst) {
    ...
    for (int64_t i = 0; i < p->len; i++) {
        if (i > 0) std::printf(", ");
        auto* b = reinterpret_cast<TythonBytes*>(static_cast<uintptr_t>(p->data[i]));
...
void TYTHON_FN(print_list_bytearray)(TythonList* lst) {
    ...
    for (int64_t i = 0; i < p->len; i++) {
        if (i > 0) std::printf(", ");
        auto* ba = reinterpret_cast<TythonByteArray*>(static_cast<uintptr_t>(p->data[i]));
