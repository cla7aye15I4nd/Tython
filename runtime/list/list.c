...
TythonList* TYTHON_FN(list_new)(const int64_t* data, int64_t len) {
    TythonList* lst = (TythonList*)TYTHON_FN(malloc)(sizeof(TythonList));
    int64_t cap = len > 8 ? len : 8;
    lst->len = len;
    lst->capacity = cap;
    lst->data = (int64_t*)TYTHON_FN(malloc)(cap * (int64_t)sizeof(int64_t));
    if (len > 0 && data) {
...
TythonList* TYTHON_FN(list_empty)(void) {
    return TYTHON_FN(list_new)(NULL, 0);
}
...
void TYTHON_FN(list_append)(TythonList* lst, int64_t value) {
    if (lst->len >= lst->capacity) {
        ...
        int64_t* new_data = (int64_t*)TYTHON_FN(malloc)(new_cap * (int64_t)sizeof(int64_t));
...
void TYTHON_FN(list_remove)(TythonList* lst, int64_t value) {
    ...
    TYTHON_FN(raise)(TYTHON_EXC_VALUE_ERROR, TYTHON_FN(str_new)("list.remove(x): x not in list", 30));
...
TythonList* TYTHON_FN(sorted_int)(TythonList* lst) {
    TythonList* out = TYTHON_FN(list_copy)(lst);
...
TythonList* TYTHON_FN(list_copy)(TythonList* lst) {
    return TYTHON_FN(list_new)(lst->data, lst->len);
}
