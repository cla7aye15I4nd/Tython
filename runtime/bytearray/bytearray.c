...
TythonByteArray* TYTHON_FN(bytearray_new)(const uint8_t* data, int64_t len) {
    TythonByteArray* ba = (TythonByteArray*)TYTHON_FN(malloc)(sizeof(TythonByteArray));
    int64_t cap = len > 8 ? len : 8;
    ba->len = len;
    ba->capacity = cap;
    ba->data = (uint8_t*)TYTHON_FN(malloc)(cap);
    if (len > 0 && data) {
...
TythonByteArray* TYTHON_FN(bytearray_empty)(void) {
    return TYTHON_FN(bytearray_new)(NULL, 0);
}
...
TythonByteArray* TYTHON_FN(bytearray_repeat)(TythonByteArray* s, int64_t n) {
    ...
    TythonByteArray* r = (TythonByteArray*)TYTHON_FN(malloc)(sizeof(TythonByteArray));
    r->len = new_len;
    r->capacity = new_len;
    r->data = (uint8_t*)TYTHON_FN(malloc)(new_len);
...
void TYTHON_FN(bytearray_append)(TythonByteArray* ba, int64_t byte_val) {
    if (ba->len >= ba->capacity) {
        ...
        uint8_t* new_data = (uint8_t*)TYTHON_FN(malloc)(new_cap);
